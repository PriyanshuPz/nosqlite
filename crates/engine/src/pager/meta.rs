...
pub const MAGIC_STR: &'static [u8; 18] = b"NOSQLITE FORMAT 01";
...
pub fn write_metadata(file: &mut File) -> Result<()> {
    ...
    page.data[cursor..cursor + 18].copy_from_slice(MAGIC_STR);
    cursor += 18;
    page.data[cursor] = VERSION;
    cursor += 1;
